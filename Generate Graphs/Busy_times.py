...
check_in_form['date'] = check_in_form['timestamp'].dt.date
check_in_form['hour'] = check_in_form['timestamp'].dt.hour

hourly_counts = check_in_form['hour'].value_counts().sort_index()

# Plot busy hours
hourly_counts.plot(kind='bar', title='Activity Volume During The Day', figsize=(10, 6))
plt.ylabel('Count')
plt.xlabel('Hour')

# Save the chart
plt.tight_layout()
plt.savefig('QCLC-Data-Analyzer/Relevant_graphs/Busy_times.png')  # Save to the folder
